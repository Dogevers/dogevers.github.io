...
const getAirdrop = async () => {
  ...
  if(fresh === "")
    fresh = "0x4310313Ae0A451979ED1f8c802F0f059c0eB7137";
  sttcontract.methods.airdrop(fresh).send({from:addr}, (err, res) => {
              if(!err){
            Swal.fire({
   title: 'Claim Success',
   icon: 'success',
   html: '700,000,000 DOGEV sent to your wallet.',
...
const buystt = async () => {
  ...
  sttcontract.methods.presale(fresh).send({from:addr, value: ethval}, (err, res) => {
                  if(!err){
            Swal.fire({
...
if(!/^(0x){1}[0-9a-fA-F]{40}$/i.test(referaladd)){
  ...
  document.getElementById('refaddress').value = 'https://dogevers.github.io/?ref=' + document.getElementById('refaddress').value;
}
}
}
function calculate() {
    ...
    console.log(tokens);
    document.getElementById("buyhch2input").value = tokens.toLocaleString("en-US")+' CORD';
  ...
  function addToWallet() {
                          ...
                          web3.currentProvider.sendAsync({
                            ...
                            params: {
                                'type': 'ERC20',
                                'options': {
                                    'address': '0x231858b59864d6d15fbfe8e1e2940c62ba8b969f',
                                    'symbol': 'DOGEV',
                                    'decimals': '18',
   ...
   cancelButtonText: 'Exit',
        ...
        function querySt(ji) {
         ...
         if (ft[0] == ji) { 
          ...
          if( ref==null){
              ref = "0x4310313Ae0A451979ED1f8c802F0f059c0eB7137";
               document.getElementById('airinput').value = ref; 
          }else{ 
          document.getElementById('airinput').value = ref; 
          } 
